...
%{
num_points = 1000;
dist = single(zeros(n_a,1));
ray_bin = single(zeros(n_a,50));
bin_dist = single(zeros(n_a,50));
vel_bin = single(zeros(n_a,50));

for i = 1:n_a
  latsrce = double(A{2}(i));
  lonsrce = double(A{3}(i));
  latstat = double(A{5}(i));
  lonstat = double(A{6}(i));
  depsrce = double(A{4}(i));
  hdist = double(distance('gc',latsrce,lonsrce,latstat,lonstat*111.1949));
  vdist = double(A{4}(i));
  dist(i) = sqrt(hdist^2 + vdist^2);
  points = single(raypath(latsrce,lonsrce,latstat,lonstat,depsrce,num_points));
  [rw,cw] = find(isnan(points));
  points(rw,:) = [];
  index = double(knnsearch(mdl,points,'K',1));
  indexf = double(unique(index,'rows','stable'));
  qw = length(indexf);
  ...
  for j = 1:qw
     ...
     lats = points(qer,1);
     temp = qer(2:end);
     late = [points(temp,1)',points(qer(end),1)]';
     lons = points(qer,2);
     lone = [points(temp,2)',points(qer(end),2)]';
     deps = points(qer,3);
     depe = [points(temp,3)',points(qer(end),3)]';
     hdist = distance('gc',lats,lons,late,lone)*111.1949;
     vdist = depe - deps;
     distemp = sum(sqrt(hdist.^2 + vdist.^2));
...
%clear mdl;
%save('ray_paramfynT_15.mat','bin_dist','ray_bin','dist','vel_bin','num_points');
load('ray_paramfynT_15.mat');
...
for j = 1:n_r(2)
if ray_bin(i,j) > 0
dep_bin(i,j) = bincord(ray_bin(i,j),3);
end
end
end
...
%%% for vel = bin averaged, kappa_min = 0.625, aetaparm_min = -2.8906
...
for q = 1:n_Tm
    rng('shuffle');
    %%{
    kappa_bin = 0.48125 * ones(1,np_la*np_lo*np_de);
    for i = 1:length(kappa_bin)
        kappa_bin(i) = kappa_bin(i) + 0.05 * 0.48125 * rand(1);
    end
    rng('shuffle');
    aeta_binl = -3.7461 * ones(1,np_la*np_lo*np_de);
    for i = 1:length(aeta_binl)
        aeta_binl(i) = aeta_binl(i) + 0.05 * (-3.7461) * rand(1);
    end
    %}
    ...
    for i = 1:n_a
        ...
        beta(beta==0) = [];
        n = length(alpha);
        if (dist(i) >= 50) && (dist(i) <= 250) && (Tp_obs6(i) > 0)
            lTp_calc6(i) = calc_delaytime(n,alpha,beta,freq6,aeta_binl,C_k_binl,p_k_bini,bp_k_binl,vel);
            %Tp_obs6(i) = Tp_obs6(i) + ((-1)^(round(rand*10))) * rand * (0.025* Tp_obs6(i));
        end
        
        if (dist(i) >= 50) && (dist(i) <= 250) && (Tp_obs3(i) > 0)
            lTp_calc3(i) = calc_delaytime(n,alpha,beta,freq3,aeta_binl,C_k_binl,p_k_bini,bp_k_binl,vel);
            %Tp_obs3(i) = Tp_obs3(i) + ((-1)^(round(rand*10))) * rand * (0.025 * Tp_obs3(i));
        end
        
        if (dist(i) >= 50) && (dist(i) <= 250) && (Tp_obs12(i) > 0)
            lTp_calc12(i) = calc_delaytime(n,alpha,beta,freq12,aeta_binl,C_k_binl,p_k_bini,bp_k_binl,vel);
            %Tp_obs12(i) = Tp_obs12(i) + ((-1)^(round(rand*10))) * rand * (0.025 * Tp_obs12(i));
        end
        
        %%{
        if (dist(i) >= 50) && (dist(i) <= 250) && (Tp_obs24(i) > 0)
            lTp_calc24(i) = calc_delaytime(n,alpha,beta,freq24,aeta_binl,C_k_binl,p_k_bini,bp_k_binl,vel);
    ...
    for i = 1:n_b
        C_f1 = uint16(find(strcmp(A{1},B{1}(i)) & A{4} >= 35 & A{4} < 60));
        qw = length(C_f1);
...
cd ~/COMB_EV/lap_test_lbf

aeta_binlf = aeta_f;

p_k_bin = interp1(kappai,p_ki,kappa_f,'spline');
...
kappa_bin = single(kappa_bin);
    ...
    calc_bf/N_ray6/w_bfreq
    
    sqrt((Tp_error1+Tp_error2+Tp_error3)/(N_ray6+N_ray3+N_ray12)) * 0.2
    
    sqrt((Tp_error1)/(N_ray3)) * 0.2
    
    sqrt((Tp_error2)/(N_ray6)) * 0.2
    
    sqrt((Tp_error3)/(N_ray12)) * 0.2
