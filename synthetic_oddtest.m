...
fid = fopen('st_delayodds.txt','r');
A = textscan(fid,'%s %f %f %f %f %f %f %f %f %f');
fclose(fid);

n_a = length(A{2});

fid = fopen('freq_fitodds.txt','r');
B = textscan(fid,'%s %f %f %f %f %f %f %f %f %f %f %f');
fclose(fid);

n_b = length(B{2});

fid = fopen('pdf_odd.txt','r');
...
%clear mdl;
%save('ray_paramfynOdd.mat','bin_dist','ray_bin','dist');
load('ray_paramfynOdd.mat');
