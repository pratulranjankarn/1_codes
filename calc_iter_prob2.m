function [post_prob,count,Temp] = calc_iter_prob2(inm,iter,Tempn,n_burn,...
    ...
    load('ray_paramfynD.mat','ray_bin','bin_dist','dist','vel_bin')
    ...
    for i = 1:length(qerr)
        ...
        if (Tp_obs6(qm) > 0) && (freq6(qm) > 0) && (dist(qm) >= 50) && (dist(qm) <= 250)
            lTp_calcn6(qm) = calc_delaytime(n,alpha,beta,freq6(qm),...
                aeta_binl2,C_k_binl,p_k_bini,bp_k_binl,vel);
        end
        
        if (Tp_obs3(qm) > 0) && (freq3(qm) > 0) && (dist(qm) >= 50) && (dist(qm) <= 250)
            lTp_calcn3(qm) = calc_delaytime(n,alpha,beta,freq3(qm),...
                aeta_binl2,C_k_binl,p_k_bini,bp_k_binl,vel);
        end
        
        if (Tp_obs12(qm) > 0) && (freq12(qm) > 0) && (dist(qm) >= 50) && (dist(qm) <= 250)
            lTp_calcn12(qm) = calc_delaytime(n,alpha,beta,freq12(qm),...
                aeta_binl2,C_k_binl,p_k_bini,bp_k_binl,vel);
        end
    ...
    Tp_errorn = Tp_error2 + Tp_error1 + Tp_error3 + Tp_error4;
    ...
    Bf_calc4 = single(zeros(n_e,1));
    
    
    for i = 1:n_b
        C_f = C_ff(i,:);
        C_f(C_f==0) = [];
        qw = single(length(C_f));
        %qw
        if qw > 0
            Rat3 = lTp_calcn3(C_f)-lTp_calcn6(C_f);
            Rat6 = single(zeros(qw,1));
            Rat12 = lTp_calcn12(C_f)-lTp_calcn6(C_f);
            Rat24 = lTp_calcn24(C_f)-lTp_calcn6(C_f);
    ...
    calc4 = sum((Bf_calc4-Bfo4).^2./Bfv4)/n_e;

    calc_bfn = w_bfreq * N_ray6 * (calc1 + calc2 + calc3 + calc4);
    ...
    save(filename,'kappa_bin','aeta_binl','post_prob'...
        ,'Temp','count','lTp_calc3','lTp_calc6','lTp_calc12','lTp_calc24','-append');
