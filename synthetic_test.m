...
for q = 1:n_Tm
    ...
    for i = 1:length(kappa_bin)
        kappa_bin(i) = kappa_bin(i) + 0.05 * rand * kappa_bin(i);
    end
    rng('shuffle');
    aeta_binl = -3.3750 * ones(1,np_la*np_lo*np_de);
    for i = 1:length(aeta_binl)
        aeta_binl(i) = aeta_binl(i) + 0.05 * rand * aeta_binl(i);
    end
    %}
    ...
    Tp_calc3 = single(zeros(n_a,1));
    Tp_calc6 = single(zeros(n_a,1));
    Tp_calc12 = single(zeros(n_a,1));
    Tp_calc24 = single(zeros(n_a,1));
    ...
    for i = 1:n_a
        ...
        beta(beta==0) = [];
        n = length(alpha);
        if (Tp_syn6(i) > 0) && (freq6(i) > 0) && (dist(i) >= 50) && (dist(i) <= 250)
            lTp_calc6(i) = calc_delaytime(n,alpha,beta,freq6(i),aeta_binl,C_k_binl,p_k_bini,bp_k_binl,vel);
            Tp_calc6(i) = 10.^lTp_calc6(i);
        end
        
        if (Tp_syn3(i) > 0) && (freq3(i) > 0) && (dist(i) >= 50) && (dist(i) <= 250)
            lTp_calc3(i) = calc_delaytime(n,alpha,beta,freq3(i),aeta_binl,C_k_binl,p_k_bini,bp_k_binl,vel);
            Tp_calc3(i) = 10.^lTp_calc3(i);
        end
        
        if (Tp_syn12(i) > 0) && (freq12(i) > 0) && (dist(i) >= 50) && (dist(i) <= 250)
            lTp_calc12(i) = calc_delaytime(n,alpha,beta,freq12(i),aeta_binl,C_k_binl,p_k_bini,bp_k_binl,vel);
            Tp_calc12(i) = 10.^lTp_calc12(i);
        end
        
        if (Tp_syn24(i) > 0) && (freq24(i) > 0) && (dist(i) >= 50) && (dist(i) <= 250)
            lTp_calc24(i) = calc_delaytime(n,alpha,beta,freq24(i),aeta_binl,C_k_binl,p_k_bini,bp_k_binl,vel);
            Tp_calc24(i) = 10.^lTp_calc24(i);
        end
        
    end
    
    Tp_calc3(lTp_calc3==0) = 0;
    Tp_calc6(lTp_calc6==0) = 0;
    Tp_calc12(lTp_calc12==0) = 0;
    Tp_calc24(lTp_calc24==0) = 0;
    ...
    Tp_error1 = sum((Tp_syn3-Tp_calc3).^2)/0.04;
    Tp_error2 = sum((Tp_syn6-Tp_calc6).^2)/0.04;
    Tp_error3 = sum((Tp_syn12-Tp_calc12).^2)/0.04;
    Tp_error4 = sum((Tp_syn24-Tp_calc24).^2)/0.04;
    ...
    Bfo1 = single(Bf_syn1);
    Bfv1 = single(Bfvd_syn1);
    Bfo2 = single(Bf_syn2);
    Bfv2 = single(Bfvd_syn2);
    Bfo3 = single(Bf_syn3);
    Bfv3 = single(Bfvd_syn3);
    ...
    sqrt((Tp_error1+Tp_error2+Tp_error3+Tp_error4)/(N_ray6+N_ray3+N_ray12+N_ray24))
...
for j = 1:n_iter
    ...
    if j > n_sig
     ...
     if j > n_burn
       ...
       if j > n_burn+1
       temp_k = (kappa_f - kappa_bin).^2;
       temp_e = (aeta_f - aeta_binl).^2;
       s_kappa = sqrt(temp_k);
       s_aeta = sqrt(temp_e);
       end
     end 
...
F = textscan(fid,'%f %f %f');
fclose(fid);
aeta_binl(bin_index) = F{3};
cd ~/COMB_EV/synthetic_test_200k/

kappa_binp = kappa_f - kappa_bin;
aeta_binlp = aeta_f - aeta_binl;
...
fid = fopen('kappa_per_final.txt','w');
fprintf(fid,'%f %f %f\n',[bincord(bin_index,1),bincord(bin_index,2),kappa_binp(bin_index)']');
fclose(fid);
...
fid = fopen('aetaparm_per_final.txt','w');
fprintf(fid,'%f %f %f\n',[bincord(bin_index,1),bincord(bin_index,2),aeta_binlp(bin_index)']');
fclose(fid);
