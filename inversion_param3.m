...
for q = 1:n_Tm
    rng('shuffle');
    %{
    kappa_bin = 0.675 * ones(1,np_la*np_lo*np_de);
    for i = 1:length(kappa_bin)
        kappa_bin(i) = kappa_bin(i) + 0.05 * 0.675 * rand(1);
    end
    rng('shuffle');
    aeta = 0.07 * ones(1,np_la*np_lo*np_de);
    for i = 1:length(aeta)
        aeta(i) = aeta(i) + 0.05 * (0.07) * rand(1);
    end
    heter = 4.25 * ones(1,np_la*np_lo*np_de);
    for i = 1:length(heter)
        heter(i) = heter(i) + 0.05 * (4.25) * rand(1);
    end
    aeta = single(aeta);
    heter = single(heter);
    %}
    ...
    load(filename,'kappa_bin','aeta','heter','post_prob'...
        ,'Temp','count');
    ...
    %aeta_binl = single(2./(p_k_bin-1) .* log10(aeta) + log10(1./heter));
    ...
    save(filename,'kappa_bin','aeta','heter','post_prob'...
...
for j = 1:n_iter
    ...
    %%{
    parfor i = 1:n_Tm
        [post_prob(i),count(i),Temp(i)] = calc_iter_prob3(i,j,Tempr(i),n_burn,...
    ...
    if j > n_sig
     ...
     if j > n_burn
       ...
       load(filename,'aeta','kappa_bin','heter');
       kappa_f = (kappa_f * (j-n_burn-1) + (kappa_bin))/(j-n_burn);
       aeta_f = (aeta_f * (j-n_burn-1) + (aeta))/(j-n_burn);
       heter_f = (heter_f * (j-n_burn-1) + (heter))/(j-n_burn);
       %{
       if j > n_burn+1
...
cd ~/COMB_EV/param_test_3

p_k_bin = interp1(kappai,p_ki,kappa_f,'spline');

aeta_binlf = single(2./(p_k_bin-1) .* log10(aeta_f) + log10(1./heter_f));

psdf = log10(8) + 1.5*log10(pi) + 2*log10(aeta_f) + 3*log10(heter_f) + ...
...
fid = fopen('aeta_f90.txt','w');
fprintf(fid,'%f %f %f\n',[bincord(sw1,1),bincord(sw1,2),aeta_f(sw1)']');
fclose(fid);
...
fid = fopen('heter_f70.txt','w');
fprintf(fid,'%f %f %f\n',[bincord(sw1,1),bincord(sw1,2),heter_f(sw1)']');
fclose(fid);
...
fid = fopen('aeta_final.txt','w');
fprintf(fid,'%f %f %f\n',[bincord(bin_index,1),bincord(bin_index,2),aeta_f(bin_index)']');
fclose(fid);
...
fid = fopen('heter_final.txt','w');
fprintf(fid,'%f %f %f\n',[bincord(bin_index,1),bincord(bin_index,2),heter_f(bin_index)']');
fclose(fid);
